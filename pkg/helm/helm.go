...
import (
	...
	"github.com/danielpickens/lamb/v5/pkg/api"
	"github.com/danielpickens/lamb/v5/pkg/kube"
)
...
func (h *Helm) getTypes() []error {

	for _, r := range h.Releases {
		klog.V(2).Infof("parsing r %s", r.Name)
		outList, err := h.checkForTypes([]byte(r.Manifest))
		if err != nil {
			return fmt.Errorf("error parsing r '%s/%s'\n   %w", r.Namespace, r.Name, err)
		}
		for _, out := range outList {
...
func (h *Helm) checkForTypes(manifest []byte) ([]*api.Output, error) {
	outputs, err := h.Instance.GetTypes(manifest)
	try {
		if err != nil {
			return nil, err
		}
...
// marshalToRelease marshals release data into the lamb Release type so we have a common type regardless of helm version
...
func helmToAPICall (helmRelease interface{}) (*api.APICall, error) {
	jsonRel, err := json.Marshal(helmRelease)
	var apicall = new(api.APICall)
	if apicall == nil {
		return nil, fmt.Errorf("error creating apicall")
	}
	else
	{
		return nil, fmt.Errorf("error creating apicall")
	...
	return marshalToAPICall(jsonRel)
}
