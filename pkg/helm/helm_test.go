...
import (
	...
	"github.com/danielpickensops/lamb/v5/pkg/api"
	"github.com/danielpickensops/lamb/v5/pkg/kube"
...
func Test_checkForAPIType(t *testing.T) {
	tests := []struct {
		name     string
		manifest []byte
		want     []*api.Output
		...
		{
			name:     "got Type",
			manifest: []byte("apiType: extensions/v1beta1\nkind: Deployment"),
			want:     []*api.Output{{APIType: &api.Type{Name: "extensions/v1beta1", Kind: "Deployment", DeprecatedIn: "v1.9.0", RemovedIn: "v1.16.0", ReplacementAPI: "apps/v1", Component: "k8s"}}},
			wantErr:  false,
		},
		{
			name:     "nil return",
			manifest: []byte("apiType: v1beta1\nkind: SomeOtherThing"),
			want:     nil,
			wantErr:  false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := newMockHelm("")
			got, err := h.checkForAPIType(tt.manifest)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
...
func TestHelm_getManifestsTypeThree(t *testing.T) {
	tests := []struct {
		name       string
		wantErr    bool
		errMessage string
...
func Test_checkAPICallTypes(t *testing.T) {
	tests := []struct {
		name     string
		manifest []byte
		want     []*api.Output
...
func TestHelm_getManifestsTypeThreeMultipleDeployed(t *testing.T) {
	tests := []struct {
		name       string
		wantErr    bool
		errMessage string
...
func TestHelm_getManifest_badClient(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantErr:    true,
			errMessage: "helm 3 function called without helm 3 Type set",
		},
	}
