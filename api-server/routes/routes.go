...
import (
	...
	"github.com/danielpickens/yeti/api-server/controllers/controllersv1"
	"github.com/danielpickens/yeti/api-server/controllers/web"
	...
	"github.com/danielpickens/yeti/common/yeticontext"
)
...
func injectCurrentOrganization(c *gin.Context) {
	orgName := strings.TrimSpace(c.GetHeader(consts.yetiOrganizationHeaderName))
	if orgName == "" {
...
func NewRouter() (*fizz.Fizz, error) {
	...
	store := cookie.NewStore([]byte(config.yetiConfig.Server.SessionSecretKey))
	if config.yetiConfig.SaasDomainSuffix != "" {
		domain, _, _ := xstrings.Partition(config.yetiConfig.SaasDomainSuffix, ":")
	...
	engine.Use(sessions.Sessions("yeti-session-v2", store))
	...
	mlGroup := engine.Group("/api/v1/ml_repositories/:mlRepositoryName/mls/:version")
	mlGroup.Use(requireLogin)

	mlGroup.PUT("/upload", controllersv1.mlController.Upload)
	mlGroup.GET("/download", controllersv1.mlController.Download)
	...
	mlRepositoryRoutes(apiRootGroup)
	...
	apiRootGroup.GET("/mls", []fizz.OperationOption{
		fizz.ID("List all mls"),
		fizz.Summary("List all mls"),
	}, tonic.Handler(controllersv1.mlController.ListAll, 200))

	apiRootGroup.GET("/image_build_status_unsynced_mls", []fizz.OperationOption{
		fizz.ID("List image build status unsynced mls"),
		fizz.Summary("List image build status unsynced mls"),
	}, tonic.Handler(controllersv1.mlController.ListImageBuildStatusUnsynced, 200))
...
func getLoginUser(ctx *gin.Context) (user *models.User, err error) {
	apiTokenStr := ctx.GetHeader(consts.yetiApiTokenHeaderName)
	...
	yeticontext.SetUserName(ctx, user.Name)
...
func organizationRoutes(grp *fizz.RouterGroup) {
	...
	grp.GET("/yeti_components", []fizz.OperationOption{
		fizz.ID("List organization all yeti components"),
		fizz.Summary("List organization all yeti components"),
	}, tonic.Handler(controllersv1.yetiComponentController.ListAll, 200))
	...
	// mlRepositoryRoutes(resourceGrp)
...
func clusterRoutes(grp *fizz.RouterGroup) {
	...
	yetiComponentRoutes(resourceGrp)
	deploymentRoutes(resourceGrp)
}

func mlRepositoryRoutes(grp *fizz.RouterGroup) {
	grp = grp.Group("/ml_repositories", "ml repositories", "ml repositories")

	resourceGrp := grp.Group("/:mlRepositoryName", "ml repository resource", "ml repository resource")

	resourceGrp.GET("", []fizz.OperationOption{
		fizz.ID("Get a ml repository"),
		fizz.Summary("Get a ml repository"),
	}, tonic.Handler(controllersv1.mlRepositoryController.Get, 200))

	resourceGrp.PATCH("", []fizz.OperationOption{
		fizz.ID("Update a ml repository"),
		fizz.Summary("Update a ml repository"),
	}, tonic.Handler(controllersv1.mlRepositoryController.Update, 200))

	resourceGrp.GET("/deployments", []fizz.OperationOption{
		fizz.ID("List ml repository deployments"),
		fizz.Summary("List ml repository deployments"),
	}, tonic.Handler(controllersv1.mlRepositoryController.ListDeployment, 200))

	grp.GET("", []fizz.OperationOption{
		fizz.ID("List ml repositories"),
		fizz.Summary("List ml repositories"),
	}, tonic.Handler(controllersv1.mlRepositoryController.List, 200))

	grp.POST("", []fizz.OperationOption{
		fizz.ID("Create ml repository"),
		fizz.Summary("Create ml repository"),
	}, tonic.Handler(controllersv1.mlRepositoryController.Create, 200))

	mlRoutes(resourceGrp)
}

func mlRoutes(grp *fizz.RouterGroup) {
	grp = grp.Group("/mls", "mls", "mls")

	resourceGrp := grp.Group("/:version", "ml resource", "ml resource")

	resourceGrp.GET("", []fizz.OperationOption{
		fizz.ID("Get a ml"),
		fizz.Summary("Get a ml"),
	}, tonic.Handler(controllersv1.mlController.Get, 200))

	resourceGrp.PATCH("", []fizz.OperationOption{
		fizz.ID("Update a ml"),
		fizz.Summary("Update a ml"),
	}, tonic.Handler(controllersv1.mlController.Update, 200))

	resourceGrp.PATCH("/update_image_build_status_syncing_at", []fizz.OperationOption{
		fizz.ID("Update a ml image build status syncing_at"),
		fizz.Summary("Update a ml image build status syncing_at"),
	}, tonic.Handler(controllersv1.mlController.UpdatemlImageBuildStatusSyncingAt, 200))

	resourceGrp.PATCH("/update_image_build_status", []fizz.OperationOption{
		fizz.ID("Update a ml image build status"),
		fizz.Summary("Update a ml image build status"),
	}, tonic.Handler(controllersv1.mlController.UpdatemlImageBuildStatus, 200))

	resourceGrp.GET("/models", []fizz.OperationOption{
		fizz.ID("List ml models"),
		fizz.Summary("List ml models"),
	}, tonic.Handler(controllersv1.mlController.ListModel, 200))

	resourceGrp.GET("/deployments", []fizz.OperationOption{
		fizz.ID("List ml deployments"),
		fizz.Summary("List ml deployments"),
	}, tonic.Handler(controllersv1.mlController.ListDeployment, 200))

	resourceGrp.PATCH("/start_multipart_upload", []fizz.OperationOption{
		fizz.ID("Start a ml multipart upload"),
		fizz.Summary("Start a ml multipart upload"),
	}, tonic.Handler(controllersv1.mlController.StartMultipartUpload, 200))

	resourceGrp.PATCH("/presign_multipart_upload_url", []fizz.OperationOption{
		fizz.ID("Presign a ml multipart upload url"),
		fizz.Summary("Presign a ml multipart upload url"),
	}, tonic.Handler(controllersv1.mlController.PreSignMultipartUploadUrl, 200))

	resourceGrp.PATCH("/complete_multipart_upload", []fizz.OperationOption{
		fizz.ID("Complete a ml multipart upload"),
		fizz.Summary("Complete a ml multipart upload"),
	}, tonic.Handler(controllersv1.mlController.CompleteMultipartUpload, 200))

	resourceGrp.PATCH("/presign_upload_url", []fizz.OperationOption{
		fizz.ID("Pre sign ml upload URL"),
		fizz.Summary("Pre sign ml upload URL"),
	}, tonic.Handler(controllersv1.mlController.PreSignUploadUrl, 200))

	resourceGrp.PATCH("/presign_download_url", []fizz.OperationOption{
		fizz.ID("Pre sign ml download URL"),
		fizz.Summary("Pre sign ml download URL"),
	}, tonic.Handler(controllersv1.mlController.PreSignDownloadUrl, 200))

	resourceGrp.PATCH("/start_upload", []fizz.OperationOption{
		fizz.ID("Start upload a ml"),
		fizz.Summary("Start upload a ml"),
	}, tonic.Handler(controllersv1.mlController.StartUpload, 200))

	resourceGrp.PATCH("/finish_upload", []fizz.OperationOption{
		fizz.ID("Finish upload a ml"),
		fizz.Summary("Finish upload a ml"),
	}, tonic.Handler(controllersv1.mlController.FinishUpload, 200))

	grp.GET("", []fizz.OperationOption{
		fizz.ID("List mls"),
		fizz.Summary("List mls"),
	}, tonic.Handler(controllersv1.mlController.List, 200))

	grp.POST("", []fizz.OperationOption{
		fizz.ID("Create a ml"),
		fizz.Summary("Create a ml"),
	}, tonic.Handler(controllersv1.mlController.Create, 200))
}

func yetiComponentRoutes(grp *fizz.RouterGroup) {
	grp = grp.Group("/yeti_components", "yeti components", "yeti components")

	grp.GET("", []fizz.OperationOption{
		fizz.ID("List yeti components"),
		fizz.Summary("List yeti components"),
	}, tonic.Handler(controllersv1.yetiComponentController.List, 200))

	grp.POST("", []fizz.OperationOption{
		fizz.ID("Register yeti component"),
		fizz.Summary("Register yeti component"),
	}, tonic.Handler(controllersv1.yetiComponentController.Register, 200))
}
...
func modelRoutes(grp *fizz.RouterGroup) {
	...
	resourceGrp.GET("/mls", []fizz.OperationOption{
		fizz.ID("List model mls"),
		fizz.Summary("List model mls"),
	}, tonic.Handler(controllersv1.ModelController.Listml, 200))
