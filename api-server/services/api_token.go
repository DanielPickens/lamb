...
import (
	...
	"github.com/danielpickens/yeti-common/config"
...
func (s *apiTokenService) GetByToken(ctx context.Context, token string) (*models.ApiToken, error) {
	...
	if len(pieces) == 3 {
		...
		expectedApiToken := ""
		if componentName != "yeti-deployment-operator" {
			yetiConf, err := config.GetyetiConfig(ctx, cliset, componentName, false)
			if err != nil {
				err = errors.Wrapf(err, "failed to get yeti config in cluster %s in organization %s", clusterName, org.Name)
				return nil, err
			}
			expectedApiToken = yetiConf.ApiToken
		} else {
			secret, err := cliset.CoreV1().Secrets(consts.DefaultKubeNamespaceyetiDeploymentComponent).Get(ctx, "env", metav1.GetOptions{})
			if err != nil {
				err = errors.Wrapf(err, "failed to get secret in cluster %s in organization %s", clusterName, org.Name)
				return nil, err
			}
			expectedApiToken = string(secret.Data[consts.EnvyetiApiToken])
		}
		if expectedApiToken == "" {
		...
		var apiToken *models.ApiToken
		apiToken, err = ApiTokenService.GetByName(ctx, org.ID, adminUser.ID, consts.yetiK8sBotApiTokenName)
		...
		if apiTokenIsNotFound {
			...
			apiToken, err = ApiTokenService.Create(ctx, CreateApiTokenOption{
				Name:           consts.yetiK8sBotApiTokenName,
				OrganizationId: org.ID,
				UserId:         adminUser.ID,
				Description:    "yeti k8s bot api token",
				Scopes:         &scopes,
			})
			if err != nil {
				var err_ error
				apiToken, err_ = ApiTokenService.GetByName(ctx, org.ID, adminUser.ID, consts.yetiK8sBotApiTokenName)
				if err_ != nil {
					err = errors.Wrapf(err, "create api token %s", consts.yetiK8sBotApiTokenName)
					return nil, err
				}
			}
		}
...
func (s *apiTokenService) List(ctx context.Context, opt ListApiTokenOption) ([]*models.ApiToken, uint, error) {
	...
	if opt.Ids != nil {
		...
		query = query.Where("name != ?", consts.yetiK8sBotApiTokenName)
	}
	var total int64
