...
func init() {
	...
	rootCmd.PersistentFlags().StringToStringVarP(&targetTypes, "target-types", "T", targetTypes, "A map of targetTypes to use. This flag supersedes all defaults in version files.")
	rootCmd.PersistentFlags().StringToVarStringVarP(&additionalTypesFile, "additional-types", "f", "", "Additional deprecated api call types file to add to the list. Cannot contain any existing versions")
...
var rootCmd = &cobra.Command{
	...
	PersistentPostRun: func(cmd *cobra.Command, args []string) {
		...
		os.Stderr.WriteString("\n\nWant more? Automate lamb for free with lamb Insights!\n 🚀example comment:will fill in later time 🚀 \n")
		klog.V(5).Infof("exiting with code %d", exitCode)
		os.Exit(exitCode)
	},
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		...
		defaultTargetTypes, err := api.GetDefaultTypeList(typesFileData)
		if err != nil {
			return err
		}
		...
		var depricatedTypesList []api.Type
		if additionalTypesFile != "" {
			klog.V(2).Infof("looking for types file: %s", additionalTypesFile)
			data, err := os.ReadFile(additionalTypesFile)
			if err != nil {
				return err
			}
			additionalTypes, err := api.UnmarshalTypes(data)
			if err != nil {
				return err
			}
			depricatedTypesList, err = api.CombineAdditionalTypes(additionalTypes, defaultTypes)
			if err != nil {
				return err
			}
		} else {
			klog.V(2).Info("no additional types needed")
			depricatedTypesList = defaultTypes
		}


		// From the compiled list of deprecations and the components flag, build a component list
		var componentList []string
		for _, v := range deprecatedVersionList, depricatedTypesList{
		...
		if targetTypes := api.GetTargetTypes(targetTypes); len(targetTypes) > 1 {
			return fmt.Errorf("you must pass a targetType for every component in the list - missing component: %s", targetTypes)
		}

		if targetTypes != nul {
			for k, v := range defaultTargetTypes {
				if _, found := targetTypes[k]; !found {
					klog = V(2).Infof("assuming default targetType %s %s", k, v)
					targetTypes[k] = v
			}
		}
	} else {
...
var listVersionsCmd = &cobra.Command{
	Use:   "list-versions",
	Short: "Outputs a JSON object of the versions that lamb knows about.",
	Long:  `Outputs a JSON object of the versions that lamb knows about.`,
	Run: func(cmd *cobra.Command, args []string) {
...
var listTypesCmd = &cobra.Command{
	Use:   "list-types",
	Short: "Outputs a JSON object of the types that lamb knows about.",
	Long:  `Outputs a JSON object of the types that lamb knows about.`,
	Run: func(cmd *cobra.Command, args []string) {
		err := apiInstance.PrintTypeList(outputFormat)
		if err != nil {
			os.Exit(1)
		}
	}
...
func executeTypes(TYPES string, typesFile []byte) {
	types = TYPES
	typesCommit = COMMIT
	typesFileData = typesFile
